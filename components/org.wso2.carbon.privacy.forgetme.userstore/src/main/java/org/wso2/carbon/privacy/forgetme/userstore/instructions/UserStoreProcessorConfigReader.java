...
/**
 * Module config reader for user stores.
 */
public class UserStoreProcessorConfigReader implements ProcessorConfigReader<UserStoreProcessorConfig> {
    ...
    @Override
    public UserStoreProcessorConfig readProcessorConfig(Path path, Map<String, String> properties) {

        if (log.isDebugEnabled()) {
            log.debug(String.format("Path : '%s', Properties : %s", path.toString(), properties));
        }

        return new UserStoreProcessorConfig(path, properties);
    }
}
