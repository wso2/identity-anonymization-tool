...
/**
 * SQL execution module to execute queries that has tenant domain appended to the username.
 */
public class TenantAppendedSQLExecutionModule implements Module<UserSQLQuery> {

    private static final Logger log = LoggerFactory.getLogger(TenantAppendedSQLExecutionModule.class);
    ...
    public TenantAppendedSQLExecutionModule(DataSourceConfig dataSourceConfig) throws SQLModuleException {
    ...
    public void execute(UserSQLQuery userSQLQuery) throws ModuleException {
        ...
        if (!StringUtils.equals(userSQLQuery.getUserIdentifier().getTenantDomain(), SUPER_TENANT)) {
            ...
            username = username + TENANT_DOMAIN_SEPARATOR + SUPER_TENANT;
        }
